...
import { CustomPopbar, type CustomPopbarRef } from "./components/popbar";
import { CustomToolbar, type CustomToolbarRef } from "./components/toolbar";
import {
...
import { CustomComment, type CustomCommentRef } from "./components/comment";
...
export class PdfjsAnnotationExtension {
  ...
  customToolbarRef: React.RefObject<CustomToolbarRef | null>; // 自定义工具栏的引用
  customPopbarRef: React.RefObject<CustomPopbarRef | null>;
  customerAnnotationMenuRef: React.RefObject<CustomAnnotationMenuRef | null>; // 自定义批注菜单的引用
  customCommentRef: React.RefObject<CustomCommentRef | null>;
  ...
  constructor() {
    ...
    this.$PDFJS_sidebarContainer =
      this.PDFJS_PDFViewerApplication.appConfig.sidebar.sidebarContainer;
    this.$PDFJS_toolbar_container =
      this.PDFJS_PDFViewerApplication.appConfig.toolbar.container;
    this.$PDFJS_viewerContainer =
      this.PDFJS_PDFViewerApplication.appConfig.viewerContainer;
    this.$PDFJS_mainContainer =
      this.PDFJS_PDFViewerApplication.appConfig.mainContainer;
    this.$PDFJS_outerContainer =
      this.PDFJS_PDFViewerApplication.appConfig.sidebar.outerContainer;
    ...
    this.appOptions = {
      ...
      [HASH_PARAMS_DEFAULT_EDITOR_ACTIVE]:
        defaultOptions.setting.HASH_PARAMS_DEFAULT_EDITOR_ACTIVE,
      [HASH_PARAMS_DEFAULT_SIDEBAR_OPEN]:
        defaultOptions.setting.HASH_PARAMS_DEFAULT_SIDEBAR_OPEN,
    };
    ...
    this.painter = new Painter({
      ...
      onAnnotationSelected: (annotation, isClick, selectorRect) => {
        ...
        if (isClick && this.isCommentOpen()) {
          ...
          this.customCommentRef.current?.selectedAnnotation(
            annotation,
            isClick
          );
        }
  ...
  private parseHashParams() {
    ...
    if (params.has(HASH_PARAMS_USERNAME)) {
      this.setOption(
        HASH_PARAMS_USERNAME,
        params.get(HASH_PARAMS_USERNAME) || ""
      );
    } else {
    ...
    if (params.has(HASH_PARAMS_GET_URL)) {
      this.setOption(
        HASH_PARAMS_GET_URL,
        params.get(HASH_PARAMS_GET_URL) || ""
      );
    } else {
    ...
    if (params.has(HASH_PARAMS_POST_URL)) {
      this.setOption(
        HASH_PARAMS_POST_URL,
        params.get(HASH_PARAMS_POST_URL) || ""
      );
    } else {
      console.warn(`${HASH_PARAMS_POST_URL} is undefined`);
    }
    if (
      params.has(HASH_PARAMS_DEFAULT_EDITOR_ACTIVE) &&
      params.get(HASH_PARAMS_DEFAULT_EDITOR_ACTIVE) === "true"
    ) {
    ...
    if (
      params.has(HASH_PARAMS_DEFAULT_SIDEBAR_OPEN) &&
      params.get(HASH_PARAMS_DEFAULT_SIDEBAR_OPEN) === "false"
    ) {
  ...
  private addCustomStyle(): void {
    ...
    this.toggleComment(
      this.getOption(HASH_PARAMS_DEFAULT_SIDEBAR_OPEN) === "true"
    );
  }
  ...
  private renderToolbar(): void {
    ...
    createRoot(toolbar).render(
      <CustomToolbar
        ...
        defaultAnnotationName={this.getOption(
          HASH_PARAMS_DEFAULT_EDITOR_ACTIVE
        )}
        defaultSidebarOpen={
          this.getOption(HASH_PARAMS_DEFAULT_SIDEBAR_OPEN) === "true"
  ...
  private renderAnnotationMenu(): void {
    ...
    this.$PDFJS_outerContainer.insertAdjacentElement(
      "afterend",
      annotationMenu
    );
    createRoot(annotationMenu).render(
      <CustomAnnotationMenu
        ...
        onChangeStyle={(currentAnnotation, style) => {
          ...
          this.customToolbarRef.current?.updateStyle(
  ...
  private updatePdfjs() {
    const currentScaleValue =
      this.PDFJS_PDFViewerApplication.pdfViewer.currentScaleValue;
    if (
  ...
  private bindPdfjsEvents(): void {
    ...
    this.PDFJS_EventBus._on("documentloaded", async () => {
      ...
      await this.painter.initAnnotations(
        data,
        defaultOptions.setting.LOAD_PDF_ANNOTATION
      );
      if (this.loadEnd) {
        this.updatePdfjs();
      }
  ...
  private async exportExcel() {
    ...
    await exportAnnotationsToExcel(
      this.PDFJS_PDFViewerApplication,
      annotations
    );
    Modal.info({
      type: "success",
