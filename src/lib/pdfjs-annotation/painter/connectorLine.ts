...
export class ConnectorLine {
  ...
  private calculateAnnotationRect(annotation: IAnnotationStore): IRect {
    const annotationNode = document.getElementById(
  ...
  private shouldDrawBasedOnDistance(
    shapeRect: IRect,
    annotationRect: IRect,
    minDistance = 30
  ): boolean {
  ...
  public drawConnection(annotation: IAnnotationStore, selectorRect: IRect) {
    requestAnimationFrame(() => {
      ...
      if (!this.connectionPathElement) {
        ...
        path.setAttribute(
          "stroke-width",
          `${defaultOptions.connectorLine.WIDTH}`
        );
        ...
        this.connectionPathElement.setAttribute(
          "stroke-dasharray",
          `${length}`
        );
        this.connectionPathElement.setAttribute(
