...
export class HighlightDecoder extends Decoder {
  ...
  public decodePdfAnnotation(
    annotation: HighlightAnnotation | StrikeOutAnnotation | UnderlineAnnotation,
    ...
    const createShape = (quadPoint: QuadPoint[]) => {
      const { x, y, width, height } = this.convertQuadPoints(
        quadPoint,
    ...
    const annotationStore: IAnnotationStore = {
      ...
      date: annotation.modificationDate || new Date().toISOString(),
      contentsObj: {
